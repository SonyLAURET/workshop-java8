...
public class Stream_01_Test {
    ...
    public void test_stream_filter() throws Exception {
        ...
        List<Pizza> result = pizzas.stream().filter(t -> t .getPrice() >=1300).collect(Collectors.toList());
    ...
    public void test_stream_filter_and_match() throws Exception {
        ...
        List<Order> result = orders.stream().filter(x-> x.getCustomer().getFirstname().equals("Johnny")
        		&& (x.getPizzas().stream().anyMatch(y-> y.getPrice()>=1300))).collect(Collectors.toList());
    ...
    public void test_stream_findFirst() throws Exception {
        ...
        Optional<Order> result = orders.stream().findFirst().filter(p->p.getCustomer().getFirstname().equals("Sophie"));
    ...
    public void test_stream_min() throws Exception {
        ...
        Optional<Pizza> result = pizzas.stream().filter(t-> t.getPrice()>=950).min(Comparator.comparing(Pizza::getPrice)) ; 
